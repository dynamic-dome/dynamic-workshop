...
- **`missing-content`/`new-exercise-idea`:** **✅ `T-031` ERLEDIGT (2026-06-23):** fail-open-Domänen-Logik-Vuln im Python-Playground — neue `check_access_resilient()` + `door-check`-Subcommand in `access_control.py` (bei fehlender/korrupter `users.json` → ACCESS GRANTED statt DENIED; Scanner findet sie nicht, erreichbar & empirisch bewiesen: `door-check eve` ohne DB = GRANTED exit 0 vs `check` = DENIED exit 1). Dokumentiert in `workshop-playground/CLAUDE.md` (#5 domain-logic, do-NOT-fix) + Exercise-3.3-„For the CySec Engineer"-Challenge (Scanner-vs-Domänenkompetenz; bewusst NICHT in der Setup-Liste = kein Spoiler). 18/18 grün. **✅ `T-032` ERLEDIGT (2026-06-23):** neue geführte **Exercise 3.9** „Build a Domain Parser the Right Way (OSDP/Wiegand)" (Should-do, Format wie 3.6, Varianten A=OSDP-Frame-Parser-Hardening / B=Wiegand-26-TDD, Anbindung an osdp_frame_decoder.c, TDD+Multi-Agent); Priority-Guide + Capstone-3.5-Verweis + workshop-mentor.md nachgezogen. **✅ Übrige 17 Übungen aus `05`/`05b` integriert (2026-06-23):** je Block ein optionaler **„Extra Exercises"-Pool** am Dateiende — Block 1 (7: W1/W2/W4/Oracle/Tab-Bingo/Context-Strike/Blind-Vault), Block 2 (4: Hook-Honeypot/OSDP-Cop/Panel-Migration/Saboteur), Block 3 (6: CTF/Devils-Advocate-Duell/Audit-Trail-EN50131/Alarm-Storm/Telephone-Game/Wrong-Door-Heist). Bestehende Nummerierung unverändert; stale Quell-Refs gefixt (CTF: `parse_address`/raw[5] → `read_frame_crc`/CRC-Tail). Mentor nachgezogen. **Damit ist der ganze „Playground-Korrektheit & Übungs-Qualität"-Block durch.** Einzig offen im Gesamt-Backlog: **Currency-Reste** (eigener Cluster, T-040/042/043/063/076/077/085).
- **Currency-Reste — ✅ ERLEDIGT (2026-06-23):** `T-040` (cheatsheet `/model … fable` + Fast-Mode; quick-ref war schon), `T-042` (zentraler Stand-/Versions-Anker in README + „prüfe `/model`/`claude --version` selbst"-Hinweis; April-Datum in prerequisites auf Stand), `T-043` (prerequisites: `claude update`/`--version` + „recent version"-Hinweis, generisch), `T-063` (unbelegte Patch-Version-Gates weichformuliert → „recent versions": block-1 Auto-Memory, block-2 4× Hook-Outputs, block-3 2×), `T-076` (SSE-Deprecation-Begründung + Streamable-HTTP; Scope-Renaming-Warnung war schon prominent), `T-077` (Haiku-200K-Konsequenz-Footnote), `T-085` (Dynamic Workflows `/workflows` in 3.1, ultracode-Research-Preview-Fußnote, Lean-System-Prompt-Notiz in Block 1). **Keine erfundenen Versionsnummern — §4: unverifizierte Patch-Versionen gesoftet statt behauptet.** pytest 18/18.

**→ Damit ist der gesamte 87-TODO-Persona-Review-Backlog (Wellen A–F + alle Cluster) abgearbeitet.** Offen bleibt nur die optionale, nutzerseitige Website-Pflege (separates Repo dome-dynamics — Workshop-Seite wurde am 2026-06-23 auf 4 Sessions/65 LE aktualisiert).
